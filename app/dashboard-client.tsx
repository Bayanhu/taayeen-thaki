...
export default function Dashboard({ user, signOutPath }: { user: UserRecord; signOutPath: string }) {
  ...
  return <div className="app" dir="rtl">
    <aside className={`sidebar ${menuOpen ? "open" : ""}`}>
      <div className="brand"><span className="brand-mark"><Sparkles size={24}/></span><div><strong>تعيين ذكي</strong><small>التربية الخاصة</small></div></div>
    ...
    <main className="main">
      <header><button className="menu" onClick={()=>setMenuOpen(!menuOpen)}><Menu/></button><p>منظومة التعيينات <span>/</span> <b>{nav.find(([id])=>id===view)?.[1]}</b></p><div className="identity"><ShieldCheck size={18}/><span>دخول آمن</span></div></header>
